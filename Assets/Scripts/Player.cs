...
public class Player : MonoBehaviour
{
            ...
            if (vector2.magnitude > 0)
            {
                ...
                if (!isCrouched)
                {
                    if (sonMarcheJoueur == false)
                    {
                        ...
                        // on met le son du bruit de pas du joueur
    ...
    public void Crouch()
    {
        ...
        {
            // on coupe le son de bruits de pas du joueur
            playerSound.Stop();
            sonMarcheJoueur = false;
    ...
    public void PlayerSendSound()
    {
        if (!isCrouched)
        {
            ...
            foreach (var colliderHit in hitColliders)
            {
                ...
                if (Physics.Raycast(raylauncher.transform.position, colliderHit.transform.GetChild(0).position - raylauncher.transform.position, out hitRobot, 12f, Wall)) // Wall
                {
